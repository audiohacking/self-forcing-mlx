"""MLX port of model/dmd.py — DMD (Distribution Matching Distillation).

Full DMD training with generator backward simulation, real/fake score models,
and timestep scheduling.
"""
...
class DMD(nn.Module):
    """Distribution Matching Distillation training module.

    Uses a generator with backward simulation, real score model (frozen),
    and fake score model (trainable) for DMD training.
    """
    ...
    def distribution_matching_loss(
        ...
        """Compute distribution matching loss with timestep scheduling."""
        if key is None:
            key = mx.random.key(0)
    ...
    def generator_loss(
        ...
        """Compute generator loss: backward simulation + DMD."""
        if key is None:
            key = mx.random.key(0)
